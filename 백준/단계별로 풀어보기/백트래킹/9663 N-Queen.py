N = int(input())
chess = [[0 for _ in range(N)] for i in range(N)] # 이중배열 만들기
result = 0

def queen(index,col): # 체스판에 퀸을 놓으면 금지되야하는 곳
    for i in range(N):
        for j in range(N):
            if i == index:
                chess[i][j]+=1
...
def reset_chess(index, col): #다시 되돌리기
    for i in range(N):
        for j in range(N):
            if i == index:
                chess[i][j]-=1
...
def check(index):
    ...
    if index == N:
        ...
        return
    for col in range(N):
        if chess[index][col]==0:
            queen(index,col)
            check(index+1)
            reset_chess(index,col)
...
# count = 0
#
# row, left, right = [0 for _ in range(n)], [0 for _ in range(2 * n - 1)], [0 for _ in
#                                                                           range(2 * n - 1)]  # 수직,왼쪽대각선,오른쪽 대각선
#
#
